...
to setup-patches
  ...
  set total_dirty  ( dirt_pct / 100 * width * height )
...
760
425
Amount of dirt the vacuum is carrying
[ amount ] of vacuum 0
17
1
15
...
This model explores the behavior of a smart vacuum cleaner with beliefs, desires and intentions. It has desires to clean all the dirts in the environment, beliefs about the location of the dirts and the garbage, and intentions to clean the nearest piece of dirt in its beliefs or go to garbage to release the dirts. It will clean all the dirts in the environment, and then stop.
This model differs from the part 2 model in that the vacuum carries a bag with a capacity in which the pieces of dirt are collected. when the collected dirts reach the bag's maximum capacity, the vacuum sets its intention to release the dirt into the garbage.

## HOW IT WORKS

The vacuum sets up its beliefs about the location of the dirts and garbage in the environment. The vacuum updates its beliefs every step to check and remove the dirts' location that has been cleaned, and accordingly updates its desire.
It has desires to clean all the dirts in the environment, when there is no dirt, the vacuum dies (disappear).
The intention is determined by its current beliefs base and the number of dirty pieces it carries. With less than capacity dirty pieces, it has the intention to collect the dirt nearest to itself in the current beliefs base. Otherwise, it has the intention to release the collected dirts into the garbage.



## HOW TO USE IT

Slider dirt_pct: Sets the dirt percent for the world Button setup: Sets up the world with the initial values defined in the function setup Button go: This triggers the agent code to run. Slider width: Sets the width of the grid. Slider height: Sets the height of the grid.
